...
namespace TIGER_BSVISION
{
    ...
    bool qImage2cvImage(const QImage &p_qimg, Mat &p_mat, int channel)
    {
        try
        {
            if (p_qimg.isNull())
            {
                Out_SyncLog << QObject::tr("图像为空无法转换");
                return false;
            }
            switch (channel)
            {
        ...
        {
            Out_SyncLog << ("qImageToCVImage error:" + QString(e.what()));
        }
        return false;
    }
    ...
    bool qRegion2cvRegion(const QSize &p_regionSize, const QPainterPath &p_path, cv::Mat &p_hRegion)
    {
        ...
        {
            Out_Warning << QObject::tr("无有效区域,无法转换");
            return false;
        }
        ...
        {
            Out_Warning << QObject::tr("图像转换失败，无法识别处理");
            return false;
        }
        return true;
    }
    ...
    cv::RotatedRect getSmallestRectangle(const cv::Mat &p_matRegion)
    {
        ...
        {
            Out_SyncLog << ("getSmallestRectangle error:" + QString(e.what()));
        }
        return minRect;
    }
    ...
    cv::Mat getSmallestRectangleRegion(const cv::Mat &p_matRegion, bool p_fill)
    {
        ...
        {
            Out_SyncLog << QString("getSmallestRectangleRegion error:").arg(QString(e.what()));
        }
        return Mat();
    }
    ...
    cv::Mat getSmallestEllipseRegion(const cv::Mat &p_matRegion, bool p_fill)
    {
        ...
        {
            Out_SyncLog << QString("getSmallestEllipseRegion error:").arg(QString(e.what()));
        }
        return Mat();
    }
    ...
    Mat getCrossContours(const QSize &p_size, const double &p_rows, const double &p_cols)
    {
        ...
        {
            Out_SyncLog << QString("getCrossContours error:").arg(QString(e.what()));
        }
        return Mat();
    }
