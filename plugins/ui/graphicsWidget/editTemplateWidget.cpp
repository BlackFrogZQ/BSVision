...
void CEditTemplateWidget::initToolActions()
{
    ...
    for (auto shape : cShape)
    {
        ...
        if (shape == CItemDef::cgitPath)
        {
            ...
            //             TIGER_Graphics::CPathItem* pPathItem = static_cast< TIGER_Graphics::CPathItem*>(pShape.pItem);
            //             pPathItem->setWidth(m_pPathWidthSlider->value());
            //             connect(m_pPathWidthSlider,&CSlider::sigValueChanged,pPathItem,&TIGER_Graphics::CPathItem::setWidth);
...
void CEditTemplateWidget::slotSetExposureTime()
{
    ...
    pWidget.setFixedSize(1000,1000);
